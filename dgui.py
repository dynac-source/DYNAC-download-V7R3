...
try:
    with open(dgpath + os.sep + "dgui.ini") as fp:  
        ...
        dynpathff=line
        dynpath = dynpathff
#        print('DBX dynac bin path=',dynpath)
        cnt = 1
        while line:
            sline=line.strip()
...
if (platform.system() == 'Windows') :
    ...
    if (last_ifpath == ""):
        last_ifpath=default_dfpath
    default_ugpath=dynpath[:-3] + "help"
#print('dbx2 user guide path',default_ugpath)
...
#       TypeCodes[0][0].split(' ',1)[0] corresponds to "INPUT"
...
#            while j < NumOfTypeCodes[i] :
#                print(i,j)
#                TC_HelpAction[i][j] = QtWidgets.QAction(TypeCodes[i][j].split(' ',1)[0], self, 
#                    triggered=(lambda: self.help_TypeCodes(TypeCodes[i][j],TypeCodeText[i][j])))       
#                j = j + 1                 
#            i = i + 1  
...
##                print(i,j)
#                TC_HelpAction[i][j] = QtWidgets.QAction(TypeCodes[i][j].split(' ',1)[0], self, 
#                    triggered=(lambda y=j: self.help_TypeCodes(TypeCodes[i][y],TypeCodeText[i][y])))       
#                j = j + 1                 
#            i = i + 1              
                  
#        print(QtWidgets.QAction(TypeCodes[0][0].split(' ',1)[0], self, 
#                    triggered=(lambda: self.help_TypeCodes(TypeCodes[0][0],TypeCodeText[0][0]))))
#        print(TC_HelpAction[0][0])      
...
#        i = 0
#        while j < NumOfTypeCodes[i] :
#            print(TypeCodes[i][j].split(' ',1)[0])
#            file_tc1_submenu.addAction(QtWidgets.QAction(TypeCodes[i][j].split(' ',1)[0], self, 
#                triggered=(lambda: self.help_TypeCodes(TypeCodes[i][j],TypeCodeText[i][j])))) 
#            j = j+1    
...
## DYNAC GUI User Guide is assumed to be in ..../dynac/help directory
        ...
        txt2= dguiv + " requires DYNAC V7R3 or newer and works with python3.10."
        abouttxt = txt1 + txt2
...
#         "Distribution files (*.dst);;Text files (*.txt);;All files (*.*)")
        filedialog = QFileDialog(self,"Open file","","Distribution files (*.dst);;Text files (*.txt);;All files (*.*)")
...
#         "Input files (*.in);;Data files (*.dat);;Text files (*.txt);;All files (*.*)")
        filedialog = QFileDialog(self,"Open file","","Input files (*.in);;Data files (*.dat);;Text files (*.txt);;All files (*.*)")
        filedialog.setFixedSize(900,440)
...
#        print("Using data from 2: ",dmpfname)
        if pfname:
            try:
                ...
                dmpDataFrame = pd.read_csv(dmpfname, skiprows=3, sep='\s+', header=None, comment='#',names=["element", "Z", "trans", "PHIs", "TOF(COG)", "Beta(COG)", "Wcog", "TOF(REF)", "Beta(REF)", "Wref", "Ex,RMS,n", "Ey,RMS,n", "El,RMS", "dWref", "EffVolt","ElementName"])
